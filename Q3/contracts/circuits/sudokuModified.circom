...
template sudoku() {
    ...
    // component check_range = RangeProof(252);

    component check_ranges_arr[9][9];
    // check_range.range[0] <== 0;
    // check_range.range[1] <== 9;

    for (var i=0; i<9; i++) {
        for (var j=0; j<9; j++) {
            check_ranges_arr[i][j] = RangeProof(252);
            
            check_ranges_arr[i][j].range[0] <== 0;
            check_ranges_arr[i][j].range[1] <== 9;
            
            check_ranges_arr[i][j].in <== puzzle[i][j];

            check_ranges_arr[i][j].out === 1;
